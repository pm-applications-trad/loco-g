...
class ImpostorSetupScreen extends ConsumerStatefulWidget {
  const ImpostorSetupScreen({super.key});

  @override
  ConsumerState<ImpostorSetupScreen> createState() => _ImpostorSetupScreenState();
}

class _ImpostorSetupScreenState extends ConsumerState<ImpostorSetupScreen> {
  int _playerCount = 4;
  int _spyCount = 1;
  int _totalRounds = 5;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l10n.translate('impostor_title')),
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                IconButton(
                  onPressed: _playerCount > AppConstants.minPlayersImpostor
                      ? () => setState(() {
                            _playerCount--;
                            _updateNameControllers();
                          })
                ...
                IconButton(
                  onPressed: _playerCount < AppConstants.maxPlayersImpostor
                      ? () => setState(() {
                            _playerCount++;
                            _updateNameControllers();
                          })
            ...
            _buildSectionTitle(context, l10n.translate('spy_count')),
            Slider(
              value: _spyCount.toDouble(),
              min: 1,
              max: max(1.0, min(3.0, (_playerCount - 2).toDouble())),
              divisions: max(0, min(3, _playerCount - 2) - 1),
              label: '$_spyCount',
              onChanged: (value) => setState(() => _spyCount = value.round()),
            ),
            const SizedBox(height: AppTheme.spacingL),
              ...
              child: ElevatedButton(
                onPressed: () {
                  final names = _nameControllers
                      ...
                      .toList();
                  ref.read(impostorGameProvider.notifier).startGame(
                        playerCount: _playerCount,
                        spyCount: _spyCount,
                        totalRounds: _totalRounds,
                        playerNames: names,
                      );
                  context.go('/impostor/play');
                },
